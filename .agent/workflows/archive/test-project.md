...
1.  **Run Lint & Typecheck**:
    - Execute `npm run lint` from `openspace-client/` to run eslint and TypeScript type checking.

2.  **Run Unit Tests**:
    - Execute `npm run test:run` from `openspace-client/` to run the complete vitest test suite.

3.  **Run E2E Tests**:
    - Execute `npm run test:e2e` from `openspace-client/` to run Chromium-only tests (standard - 38 tests)
